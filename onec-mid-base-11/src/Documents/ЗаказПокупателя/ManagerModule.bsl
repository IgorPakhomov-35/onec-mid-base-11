...
КонецПроцедуры
	...
	// Процедура-обработчик подсистемы "Печать" БСП.
	// Добавляет команды печатных форм в подменю "Печать" формы документа/списка.
	//
	// Параметры:
	//  КомандыПечати - ТаблицаЗначений - таблица команд, в которую нужно добавить новую команду.
	//
...
КонецПроцедуры
	...
	// Формирует печатные формы (Заказ покупателя, Счет на оплату, Гарантийное письмо) по заданным объектам.
	// Используется в подсистеме "Печать".
	//
	// Параметры:
	//  МассивОбъектов - Массив из ДокументСсылка, СправочникСсылка - ссылки на документы/справочники, которые нужно распечатать.
	//  ПараметрыПечати - Структура - дополнительные параметры, переданные из формы.
	//  КоллекцияПечатныхФорм - ТаблицаЗначений - для внешней печатной формы (БСП 3.x+).
	//  ОбъектыПечати - СписокЗначений из Строка,Число - (выходной параметр) для вставки областей.
	//  ПараметрыВывода - Структура - (выходной параметр) настройки печати.
	//
